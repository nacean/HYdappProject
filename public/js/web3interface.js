...
const contract_address = "0x200d6393bC10010cf99Fe1A8e7B55d82A8D9e372"; // 따옴표 안에 주소값 복사 붙여넣기
...
const _shareRoom = async (name, location, price) => {
  ...
  await roomContract.methods.shareRoom(name,location,price).send({from : user, gas : 3000000}).catch((e)=>{alert(e)});
  alert("Success to Share !");
  _updateRooms();
  await _updateUserBalance(user);
}
...
const _getMyRents = async () => {
  // 내가 대여한 방 리스트를 불러온다.
  const roomContract = await getRoomShareContract();
  const tempMyRents = await roomContract.methods.getMyRents().call({from : user, gas : 3000000});
  console.log(tempMyRents);
  const myRents = tempMyRents.map((mr)=>{return {...mr}});
  return myRents;
}
...
const _getAllRooms = async () => {
  ...
  const roomContract = await getRoomShareContract();
  const tempRooms = await roomContract.methods.getAllRooms().call({from : user, gas : 3000000});
  const rooms = tempRooms.map((room)=>{return {...room}});
  return rooms;
}
...
const _rentRoom = async (roomId, checkInDate, checkOutDate, price) => {
  ...
  // 화면을 업데이트 한다.
  const roomContract = await getRoomShareContract();
  try{await roomContract.methods.rentRoom(Number(roomId),checkInDate,checkOutDate).send({from : user, gas : 3000000, value : web3.utils.toWei(String(price),'finney')});
  alert('success to rent');}
  catch(error){
     const msg = error.message;
     console.log(msg);
     if(msg.indexOf("EVM") !== -1){
        _recommendDate(Number(roomId),checkInDate,checkOutDate);
     }
     else if(msg.indexOf("value") !== -1){
        alert("don't have enough balance to purchase");
     }
  }
...
const _recommendDate = async (roomId, checkInDate, checkOutDate) => {
  ...
  const roomContract = await getRoomShareContract();
  const rents = await roomContract.methods.recommendDate(Number(roomId),checkInDate,checkOutDate).call({from : user, gas : 3000000});
  alert("there is rent already\n" + "check-in-date : " + dateFromDay(currentYear,rents[0]) + "\n check-out-date : " + dateFromDay(currentYear,rents[1]));
}
...
const getRoomRentHistory = async () => {
  ...
  const jsonobj = returnOptionsJSON();
  const roomId = jsonobj.id;
  const tempHistory = await roomContract.methods.getRoomRentHistory(Number(roomId)).call({from : user, gas : 3000000});
  const history = tempHistory.map((ht)=>{return {...ht}});
  for(let i=0;i<history.length;i++){
     history[i].checkInDate = dateFromDay(currentYear,history[i].checkInDate);
     history[i].checkOutDate = dateFromDay(currentYear,history[i].checkOutDate);
  }
...
const markRoomAsInactive = async () => {
  const tempRoomId = document.getElementById('roomId').value;
  console.log(tempRoomId);
  await _markRoomAsInactive(Number(tempRoomId));
}
...
const _markRoomAsInactive = async (_roomId) => {
  ...
  const roomContract = await getRoomShareContract();
  try{
  await roomContract.methods.markRoomAsInactive(_roomId).send({from : user, gas : 3000000});
    alert("room InActive Confirm");
  }
  catch(e){
  	alert("you are not owner of room");
  }
  _updateRooms();
}
...
const initializeRoomShare = async () => {
  const tempRoomId = document.getElementById('roomId').value;
  await _initializeRoomShare(Number(tempRoomId));
}
...
const _initializeRoomShare = async (_roomId) => {
  ...
  const roomContract = await getRoomShareContract();
  try{
  await roomContract.methods.initializeRoomShare(_roomId).send({from : user, gas : 3000000});
    alert("room initialize Confirm");
  }
  catch(e){
  	alert("you are not owner of room");
  }
